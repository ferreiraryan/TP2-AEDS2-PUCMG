...
int main() {
  ...
  if (num_selecionados > 0) {
    int max_cilindros = selecionados[0].cilindros;
    for (int i = 1; i < num_selecionados; i++) {
      if (selecionados[i].cilindros > max_cilindros) {
        max_cilindros = selecionados[i].cilindros;
      }
    }

    int *count = calloc(max_cilindros + 1, sizeof(int));
    Veiculo *output = malloc(num_selecionados * sizeof(Veiculo));

    for (int i = 0; i < num_selecionados; i++) {
      count[selecionados[i].cilindros]++;
    }

    for (int i = 1; i <= max_cilindros; i++) {
      count[i] += count[i - 1];
    }

    for (int i = num_selecionados - 1; i >= 0; i--) {
      output[count[selecionados[i].cilindros] - 1] = selecionados[i];
      count[selecionados[i].cilindros]--;
    }

    for (int i = 0; i < num_selecionados; i++) {
      selecionados[i] = output[i];
    }
