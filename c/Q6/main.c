...
void countSortRadix(Veiculo arr[], int n, int exp) {
  Veiculo *output = malloc(n * sizeof(Veiculo));
  int i, count[10] = {0};

  for (i = 0; i < n; i++) {
    count[(arr[i].ano / exp) % 10]++;
  }
  ...
  for (i = n - 1; i >= 0; i--) {
    output[count[(arr[i].ano / exp) % 10] - 1] = arr[i];
    count[(arr[i].ano / exp) % 10]--;
  }
...
void radixSort(Veiculo arr[], int n) {
  if (n <= 0)
    return;

  int maxAno = arr[0].ano;
  for (int i = 1; i < n; i++) {
    if (arr[i].ano > maxAno) {
      maxAno = arr[i].ano;
    }
  ...
  for (int exp = 1; maxAno / exp > 0; exp *= 10) {
...
int main() {
  ...
  radixSort(selecionados, num_selecionados);
